...
export function useVisitor(){
  ...
  const refresh=React.useCallback(async()=>{if(!session)return null;try{const p=await request('/me/query',{method:'POST',data:{},token:session.token});setProfile(p);setError('');return p;}catch(e){setError(e.message);return null;}},[session]);
  ...
  const perform=async(action,data={})=>{if(!session)throw new Error('请先登录');if(lock.current)throw new Error('上一项操作仍在处理中');lock.current=true;setBusy(true);setError('');try{const signature=JSON.stringify([session.token,action,data]);if(action==='ticket'||action==='privacy'){if(!submissionKeys.has(signature))submissionKeys.set(signature,`${Date.now()}-${Math.random().toString(36).slice(2)}-submit`);data={...data,idempotencyKey:submissionKeys.get(signature)};}const r=await request('/me/action',{method:'POST',data:{...data,action},token:session.token});submissionKeys.delete(signature);await refresh();return r;}catch(e){setError(e.message);throw e;}finally{lock.current=false;setBusy(false);}};
  const logout=async()=>{setBusy(true);try{if(session)await request('/auth/logout',{method:'POST',data:{},token:session.token});}finally{sessionStorage.removeItem(KEY);setSession(null);setProfile(null);setBusy(false);}};
