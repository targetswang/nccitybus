...
Page({
    ...
    async cancel(e: any) { const s = readSession(); if (!s)
        return this.login(); if (!await confirm('确认取消报名？'))
        return; try {
    ...
    async register(e: any) { const session = readSession(); if (!session) {
        this.login();
        return;
    } const id = e.currentTarget.dataset.id; if (!await confirm('确认报名此免费活动？这不是公交预约。'))
        return; try {
