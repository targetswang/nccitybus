...
Page({
  ...
  async join(e:any){const session=readSession();if(!session){this.login();return;}const id=e.currentTarget.dataset.id;if(!await confirm('确认自愿加入该免费会员方案并同意规则？'))return;try{await meAction(session.session,'join',{id,accepted:true});void track('member_join',{page:'member',objectType:'membershipPlan',objectId:id});this.setData({note:'会员记录已保存'});await this.onShow();}catch(err:any){this.setData({error:err.message});}}
});
