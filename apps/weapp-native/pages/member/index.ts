...
Page({
    ...
    async onShow() { const session = readSession(); this.setData({ session, loading: true, error: '' }); try {
        const [content, profile] = await Promise.all([getContent(), session ? readProfile() : Promise.resolve(null)]);
        this.setData({ plans: (content.membershipPlans || []).map((p: any) => { const m = profile?.memberships?.find((m: any) => m.planId === p.id && m.status === 'active' && (!m.expiresAt || m.expiresAt > Date.now())); return { ...p, membership: m, expiry: m?.expiresAt ? new Date(m.expiresAt).toLocaleDateString() : '长期有效' }; }), profile });
    }
    catch (e: any) {
    ...
    async leave(e: any) { const s = readSession(); if (!s)
        return this.login(); if (!await confirm('确认退出该会员方案？'))
        return; try {
    ...
    async join(e: any) { const session = readSession(); if (!session) {
        this.login();
        return;
    } const id = e.currentTarget.dataset.id; if (!await confirm('确认自愿加入该免费会员方案并同意规则？'))
        return; try {
