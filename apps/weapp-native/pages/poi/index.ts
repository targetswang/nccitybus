...
Page(retainPage({ data: { poi: null as any, liked: false, error: '' }, query: {} as any, onLoad(q: any) { this.query = q; }, async onShow() {
        const q = this.query;
        this.setData({ error: '' });
        try {
            ...
            this.setData({ poi, liked, canNavigate: !!navigationTarget(poi) });
        ...
        const id = this.data.poi?.nodeId;
        if (id)
            wx.navigateTo({ url: `/pages/station/index?id=${encodeURIComponent(id)}` });
    }, nav() {
        const p = this.data.poi;
        if (p?.mapPoint?.crs === 'GCJ02')
            openPoiLocation(p.name, p.mapPoint.lat, p.mapPoint.lng);
        else
            wx.showToast({ title: '精确坐标尚未核验', icon: 'none' });
    }, async fav() {
        const p = this.data.poi, s = readSession();
        try {
            if (s) {
