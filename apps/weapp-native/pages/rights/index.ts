...
Page({ data: { benefits: [] as any[], profile: null as any, session: null as any, error: '', note: '', id: '' }, onLoad(q: any) { this.setData({ id: q.id || '' }); }, async onShow() { if (this.data.id !== undefined)
        await this.refresh(); }, async refresh() { const session = readSession(); this.setData({ session, error: '' }); try {
        const [c, p] = await Promise.all([getContent(), session ? readProfile() : Promise.resolve(null)]);
        this.setData({ benefits: (c.benefits || []).filter((b: any) => !this.data.id || b.id === this.data.id), profile: p });
        if (this.data.id && !this.data.benefits.length)
            throw new Error('权益不存在或已下线');
    }
    catch (e: any) {
        this.setData({ error: e.message });
    } }, login() { wx.navigateTo({ url: '/pages/login/index' }); }, async claim(e: any) { const s = readSession(); if (!s) {
        this.login();
        return;
    } const id = e.currentTarget.dataset.id; if (!await confirm('确认按页面规则领取此权益？'))
        return; try {
    ...
    } }, async open(e: any) { const s = readSession(); if (!s)
        return; try {
