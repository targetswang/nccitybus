...
Page({ data: { station: null as any, index: 0, pois: [] as any[], stops: [] as any[], error: '', transitError: '' },
    query: {} as any, onLoad(q: any) { this.query = q; }, async onShow() {
        const q = this.query;
        this.setData({ error: '', station: null });
        try {
            ...
            try {
                ...
                this.setData({ stops: stationMatches(s, q.id).map(s => ({ ...s, canNavigate: !!navigationTarget(s) })) });
            }
            catch (e: any) {
    ...
    nav(e: any) { const target = navigationTarget(this.data.stops[Number(e.currentTarget.dataset.index)]); if (target)
