...
**以下问题描述是 f932e6c 的历史缺陷基线。** 首轮发现 R1～R5，整理登录指南时补充发现 R6；后续已完成六项代码修复。当前结果见 [修复验收记录](REVIEW_FIXES_2026-09-20.md)，现场验收范围单独标明。
...
| ID | 优先级 | 问题 | 建议负责方向 | 状态 |
|---|---|---|---|---|
| R1 | P1 | 验证码错误次数被事务回滚 | 后端 / 认证 | 已修复；自动回归已通过 |
| R2 | P1 | 管理员旧会话不受降权/禁用影响 | 后端 / RBAC | 已修复；自动回归已通过 |
| R3 | P1 | 后台发布切换为生产不接受的 reference | 后端 / 内容发布 | 已修复；自动回归已通过 |
| R4 | P2 | offline 状态丢失，保存为 active | 后端 / 内容编辑 | 已修复；自动回归已通过 |
| R5 | P2 | 站点 revision 返回值与数据库不一致 | 后端 / 数据库 | 已修复；自动回归已通过 |
| R6 | P1 | 游客客户端 session/token 字段不一致 | H5 + 小程序 + API | 已修复；自动回归已通过 |
...
原定验收要求（已实现的自动证据和仍需现场验证的部分见修复验收记录）：
...
|---|---|---|---|---|
| R1 | Codex | 本 PR 修复提交 | REVIEW_FIXES_2026-09-20.md | 代码及自动回归通过 |
| R2 | Codex | 本 PR 修复提交 | REVIEW_FIXES_2026-09-20.md | 代码及自动回归通过 |
| R3 | Codex | 本 PR 修复提交 | REVIEW_FIXES_2026-09-20.md | 代码及自动回归通过 |
| R4 | Codex | 本 PR 修复提交 | REVIEW_FIXES_2026-09-20.md | 代码及自动回归通过 |
| R5 | Codex | 本 PR 修复提交 | REVIEW_FIXES_2026-09-20.md | 代码及自动回归通过 |
| R6 | Codex | 本 PR 修复提交 | REVIEW_FIXES_2026-09-20.md | 代码及自动回归通过 |
