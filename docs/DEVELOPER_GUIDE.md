...
本次交付为文档完善，未修复业务代码、未配置线上账号、未部署预览。先阅读本指南，再按 [审查问题清单](CODE_REVIEW_2026-09-20.md) 修复和验收。当前仍是候选工程。
...
验证码挑战有效期为 5 分钟，同一手机号再次获取需间隔至少 60 秒。代码预期限制输错 5 次，但目前限制失效，见 R1。游客会话有效期 7 天，后台 8 小时；二者用途隔离，游客登录不会自动获得后台会话。

**游客端当前有 R6：登录响应返回 token，但部分客户端读取 session。** 因此填对手机号和验证码后，也可能在账户查询、收藏、反馈等操作中收到 401。请先修复，不要归因为验证码错误。
...
配置哈希必须为 SHA-256(`phone:` + 完整手机号)，不是裸手机号哈希。更换角色应使用后台工作人员权限功能；当前管理员撤权存在 R2，应先修复后再验收。
...
| 后台发布后生产内容 503 | R3：reference 覆盖已审核发布版本 |
| 内容下架无效 | R4：status 被 merge 删除 |
...
| 六项审查问题 | 修复 commit、回归用例、运行结果 | 全部待修复 |
| 三端用户闭环 | 登录→账户→收藏/反馈→后台回复→用户回读 | 待端到端验收；R6 阻塞游客会话链路 |
| 正式内容发布 | 草稿隔离、审核、发布、双端版本、回滚 | R3/R4/R5 待修复 |
| 在线 H5 | 可访问 URL、部署 commit、手机/PC 检查记录 | 未验证 |
| 外部集成 | 微信双端真机、短信真实收码、AI、IVY、目标 PostgreSQL | 见 KNOWN_LIMITATIONS，不能以测试码替代 |

执行顺序：先修复 P1 → 修复 P2 → build/check/test → 三端实际联调 → 外部凭据和设备验收 → 发布门禁。研发负责人分配实际执行人；当前文档不虚构人员认领或截止时间。
